...
function G2PCustomerTable ({ loading, error, View, notFound, searchParams, setSearchparams }) {
    ...
    return (
        <>
                    ...
                    : <tbody className='text-neutral-primary whitespace-nowrap text-[14px] leading-[24px] font-[400]'>
                        {View?.sheets?.map((item, index) => (
                            <tr key={index} className='border-b border-neutral-outline h-[48px]'>
                                ...
                                <td className='py-2 px-[10px]'>{formatTimestamp(item?.uploaded_date)}</td>
                                ...
                                <td data-testid="amount" title={item?.transfered_amount}
                                    className='py-2 px-[10px] truncate min-w-[100px] max-w-[100px]'>{`${item?.transfered_amount.toLocaleString()}.00 MWK`}</td>
            ...
            {!notFound && error &&
                (<NoDataError heading='There are no G2P profile to view yet' />)
            }
            {
                View?.length === 0 && !loading &&
                (searchParams.get('status') !== null || searchParams.get('search') !== null) &&
                (<NoDataError className='h-tableHeight' heading='There are no G2P profile to view yet' />)
            }
        </>
    );
}
