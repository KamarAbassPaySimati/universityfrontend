Feature: Paymaart- Admin Web- log view
    ...
    Scenario: View the pay in list transaction view
        ...
        Then I should see table header containing '["Date/Time","Transaction ID","Beneficiary Paymaart ID","Entry By","Bank Name", "Transaction POP", "Amount"]'

    Scenario: View the pay out list transaction view
        Given I am in transactions log page 
        When I click on pay out tab 
        Then I should see table header containing '["Date/Time","Transaction ID","Beneficiary Paymaart ID","Entry By","Bank Name", "Transaction POP", "Amount","Status"]'

        Scenario: Search for non existing record in pay in transaction log
            Given I am in transactions log page 
            When I click on pay in tab
            When I search for particular transaction logs as "00000000000"
            Then I should read a message stating that "No data found"
            And I should read a message stating that "Try adjusting your search to find what you’re looking for"
    ...
    Scenario: Sort functionality for pay in
        ...
        Then I should see the transaction logs sorted in ascending order based on "Submission Date"
        When I click on the sort by "Submission Date"
        Then I should see the transaction logs sorted in descending order based on "Submission Date"
