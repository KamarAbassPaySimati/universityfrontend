Feature: Paymaart- Admin Web- Add Capital Bank Transactions
  ...
  @perform_logout @add_admin_user @create_new_user_and_login
  ...
  # since the PTS 2 & PTBU3 bank is not there in trust banck commented
  ...
  Scenario: Add transaction for capital bank with valid details for Inflow For EM/Funding for Transaction fee and Commission
    ...
    Scenario Outline: Add transaction for capital bank with invalid details for Payout to Paymaart Operations for excess Float in PMCA to PTBA1
        When I select the transaction type as "Payout to Paymaart Operations for excess Float in PMCA to PTBA1"
        When I enter the transaction amount as <amount> for capital bank transaction
        When I should see the entry by field should be disabled for add capital bank transaction
        When I enter the transaction POP Ref.No as <transaction_pop_ref_no>
        When I upload the transaction POP document as <POP_document>
    ...
    Scenario: Add transaction for capital bank with valid details for Payout to Paymaart Operations for excess Float in PMCA to PTBA1
        When I select the transaction type as "Payout to Paymaart Operations for excess Float in PMCA to PTBA1"
        When I enter the transaction amount as "100" for capital bank transaction
        When I should see the entry by field should be disabled for add capital bank transaction
        When I enter the valid transaction POP Ref.No
