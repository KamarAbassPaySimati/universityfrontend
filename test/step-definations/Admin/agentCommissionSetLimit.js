...
Given('I navigate to agent commission setting screen', async function () {
    await driver.get('http://localhost:3000/financials/commissions');
    await new Promise(resolve => setTimeout(resolve, 4000));
});

Then('I should see the commission conversion duration fields prefilled', async function () {
    const element = await driver.wait(until.elementLocated(By.css('[data-testid="agentCommission"]')));
    const elementValue = element.getText();
