  ...
  Then('I should see {string} graph',async function (graphType) {
    switch(graphType){
        case "Agent Registrations":
            const agentGraph = await driver.wait(until.elementLocated(By.css('[data-testid="agent-registartion"]')));
            await driver.wait(until.elementIsVisible(agentGraph));

        case "Customer Registrations":
            const customerGraph = await driver.wait(until.elementLocated(By.css('[data-testid="customer-registartion"]')));
