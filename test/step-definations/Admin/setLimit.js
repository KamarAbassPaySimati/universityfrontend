...
Given("I navigate to set limit screen", async function(){
    await driver.get("http://localhost:3000/setLimit")
...
Then("I should see a static text stating {string}",async function(string){
    let element = await driver.wait(until.elementLocated(By.css('[data-testid="staticMessage"]')))
...
When("I click the update set limit button",async function(){
    await driver.wait(until.elementLocated(By.css('[data-testid="setLimitUpdateButton"]'))).click();
    await new Promise(resolve => setTimeout(resolve, 500));
})
