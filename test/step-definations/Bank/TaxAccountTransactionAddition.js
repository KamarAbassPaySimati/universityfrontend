...
Then("I should see the enter by field is prefilled", async function(){
    let enterByTextField = await driver.wait(until.elementLocated(By.css('[data-testid="enterByField"]')));
    let isEnable = await enterByTextField.isEnabled()
    assert.notEqual(isEnable, false)
    let enterByValue = await enterByTextField.getAttribute('value');
...
When("I enter the transaction amount as {string}", async function(amount){
    let amountField = await driver.wait(until.elementLocated(By.css('[data-testid="transactionAmountField"]')));
...
When("I enter the transaction POP ref. no as {string}", async function(transactionPOPNo){
    let transactionPOPNoField = await driver.wait(until.elementLocated(By.css('[data-testid="transactionPOPNoField"]')));
