...
When('I select the transaction type as {string}', async function (transactionType) {
    ...
    if (transactionType !== '') {
        ...
        case 'Charge for Bank Services or Transactions by PTBA1 | EM credit to PMCAT':
            dropdownElement = await driver.wait(until.elementLocated(By.xpath('//*[text()="Charge for Bank Services or Transactions from PTBA1 | RM debit"]')));
            await driver.wait(until.elementIsVisible(dropdownElement));
            break;
        ...
        case 'Pay-out to Agent Post Deactivation from PTBA1 | EM credit to PMCAT':
            dropdownElement = await driver.wait(until.elementLocated(By.xpath('//*[text()="Pay-out to Agent Post Deactivation from PTBA1 | EM credit to PMCAT"]')));
            await driver.wait(until.elementIsVisible(dropdownElement));
            break;
